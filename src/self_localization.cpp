...
std::vector<Pixel> RobotLocalization::bresenham(const Pixel& start,
                                                    const Pixel& end) {
...
LaserScanSample RobotLocalization::transformPointCloud(
...
RobotLocalization::RobotLocalization() {
  ...
  scan_sub_ = nh.subscribe("/scan", 10,
                           &RobotLocalization::scanCallback, this);
  odom_sub_ = nh.subscribe("/odom", 10,
                           &RobotLocalization::odomCallback, this);
  ...
  CallbackType cb = boost::bind(&RobotLocalization::reconfigureCB,
                                this, _1, _2);
  dynamic_srv_->setCallback(cb);
}

void RobotLocalization::reconfigureCB(Config& config, uint32_t level) {
...
void RobotLocalization::handleLaserScan() {
 // mtx.lock();
...
void RobotLocalization::scoreLaserScanSamples() {
...
double RobotLocalization::scoreASample(const LaserScanSample& sample,
...
bool RobotLocalization::receiveLocalization(
...
void RobotLocalization::computeCaches() {
...
void RobotLocalization::inflateMap() {
...
void RobotLocalization::enqueueObstacle(const unsigned int index,
...
inline double RobotLocalization::distanceLookup(
...
inline signed char RobotLocalization::costLookup(
  const unsigned int mx, const unsigned int my,
  const unsigned int sx, const unsigned int sy) const {
  const unsigned int dx = abs(static_cast<int>(mx) - static_cast<int>(sx));
  const unsigned int dy = abs(static_cast<int>(my) - static_cast<int>(sy));
...
void RobotLocalization::debugAPosition(const geometry_msgs::Pose2D& pose) {
  // handleLaserScan();
 // mtx.lock();
...
void RobotLocalization::tracking() {
  ...
  while (true && ros::ok()) {
    ...
    if (big_angle_twist_) {
      ...
      if (current_position_score_ < 0.2)
        ++num;
      else
      {
        num = 0;
